...
export default {
  ...
  methods: {
    async signIn() {
        try{
            this.user = await this.$Amplify.Auth.signIn(this.username, this.password)
    ...
    async signUp () {
        ...
        try{
            ...
            this.$snotify.success('Sign up succesfull!', 'Check your email for an confirmation!',{timeout:10000});
            this.loading = false
